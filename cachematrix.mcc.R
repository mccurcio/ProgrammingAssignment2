...
makeCacheMatrix <- function(x = matrix()) {
      ...
      # The `list` command brings all these separate functions together
      # so that they can be called with object oriented properties.
...
cacheSolve <- function(x, ...) {
      m <- x$getsolve() # Inside the `getsolve()` function,
                        # the argument m gets overwritten with the value
                        # of the inverse matrix.
      if (!is.null(m)) { # "If the inverse has already been calculated,
                         # then the cachesolve should retrieve/return the
                         # value of m (the inverse matrix) from the cache."
            message("getting cached data")
            return(m)
      }
      data <- x$get() # This `get()` function returns the value of the
                      # 'original' matrix as an object to be fed into `solve()`.
      m <- solve(data, ...) # Solves using the 'original' matrix and saves the
                            # value into local memory,m.
      x$setsolve(m) # The `setsolve()` function attaches the inverse matrix
                    # calculated above and saves it to the object list.
      m
}
